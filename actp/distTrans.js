...
//    ACTP.DT = ACTP.DT || {};
    ...
    ACTP.computeDistTrans = function(inGreyImg, dt) {

        "use strict";
        var g, m, n, maxGVal, i, j, q, s, t, u, w;

        g = new ACTP.AcGrey16Image(inGreyImg.width, inGreyImg.height);
    //    g = $.extend(g, (JSON.parse(JSON.stringify(inGreyImg)))); // copies the buffer
        ...
        for (j = 0; j < n; ++j) {
            ...
            for (u = 1; u < m; ++u) {
                while (q >=0 && ACTP.EDT_f(g, j, t[q], s[q]) > ACTP.EDT_f(g, j, t[q], u)) {
                    --q;
                }
            ...
            for (u = m; u >=0; --u) {
                ...
                dt.data[j*dt.width + u] = ACTP.EDT_f(g, j, u, s[q]);
                if (u === t[q]) {
                    --q;
                }
            }
