...
(function(ACTP) {
    ...
    ACTP.Path.prototype.scaleSimpleSegs = function(scale) {
    ...
    ACTP.Path.prototype.buildThreeShape = function(pathSeg) {
    ...
    ACTP.Path.prototype.addSimpleSegsAsMeshes = function(matThick) {
    ...
    ACTP.Path.prototype.addSimpleSegsToScene = function() {
    ...
    ACTP.Path.prototype.buildSimpleSegs = function(err) {
    ...
    ACTP.Path.prototype.buildArcInterp = function(err) {
    ...
    ACTP.Path.getGCodeMoveTo = function(pt, zVal, pixelsPerMm) {
    ...
    ACTP.Path.prototype.gcodeForSegment = function(pathSegment, jogHeight, cutHeight, pxPerMm) {
              
        var gCode = "";
        gCode += "G0 Z" + jogHeight.toString() + "\n"; // pull up to ceiling
        var jog = pathSegment[0];
        dataStr = ACTP.Path.getGCodeMoveTo(jog, jogHeight, pxPerMm);
        gCode += dataStr + "\n";
        
        pathSegment.forEach(function(ptDir, iPtDir) {
        
            dataStr = ACTP.Path.getGCodeMoveTo(ptDir, cutHeight, pxPerMm);
            gCode += dataStr + "\n";
        });
    ...
    ACTP.Path.prototype.gCodeForSegs = function (pathSegs, jogHeight, cutHeight, pxPerMm) {

        var gCode = "";
        for (var iSeg = 0; iSeg < pathSegs.length ; ++iSeg) {
            
            var pathSeg = pathSegs[iSeg];
            gCode += this.gcodeForSegment(pathSeg, jogHeight, cutHeight, pxPerMm); 
        }
        return gCode;
    }

    ACTP.Path.prototype.generateGCode = function(pxPerMm, useSimplifiedSegs) {
        ...
        gCode += webCAM.gCodeSetUp(feedrateInPerMin);
        ...
        gCode += webCAM.gCodeTakeDown(jogHeight);
        
        return gCode;
    }

    ACTP.Path.prototype.drawSegments = function(canvas) {
    ...
    // utility that has nothing to do with paths per se
    ACTP.Path.prototype.draw2DPtArrayWithLines = function(ctx, pts) {
    ...
    ACTP.Path.prototype.draw2DPtArrayWithRects = function(ctx, pts) {
        ...
        for (i=0; i < pts.length-1; ++i) {
            ...
            iPt = pts[i];
            ctx.rect(iPt.x-2, iPt.y-2, 4,4);  
        }
        
        ctx.fill();
    }

    ACTP.Path.prototype.drawSimpleSegments = function(canvas) {
        ...
        for (i=0; i < end; ++i) {
            this.draw2DPtArrayWithRects(ctx, this.pathSimpleSegs[i]);
        }
    }

    ACTP.Path.prototype.drawArcSegs = function(canvas) {
        ...
        for (i=0; i < end; ++i) {
            this.drawPtsWithArcs(ctx, this.pathArcSegs[i]);
        }
    }
        
    ACTP.Path.prototype.drawPtsWithArcs = function(ctx, pts) {
    ...
    ACTP.Path.prototype.draw = function(canvas) {

        var i, iPtDir;
        var ctx = canvas.getContext('2d');
        
        this.drawPtDirs(ctx, this.ptdirs);   
        //this.drawSimpleSegments(canvas);
    }   
